...
const Register = () => {
  ...
  const { createUser, updateUser, setUser, signInGoogle, signIn } = useAuth();
  const axiosSecure = useAxiosSecure();

  const handleDemoLogin = async (role) => {
    const email = role === "admin" ? "admin@admin.com" : "student@student.com";
    ...
    try {
      ...
      Swal.fire({
        position: "center",
        icon: "success",
        title: `Logged in as Demo ${role === "admin" ? "Admin" : "Student"}`,
      ...
      Swal.fire({
        icon: "error",
        title: "Login Failed",
        text: "Could not log in with demo credentials",
      ...
      <div className=" font-display text-text-main flex items-center justify-center p-6 flex-1 ">
        <div className="w-full max-w-[500px] mx-auto ">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 border border-border-light rounded-lg py-2.5 px-4 transition-colors duration-200 mb-4 group hover:bg-primary disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {googleLoading ? (
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            <button
              disabled={loading}
              className="w-full bg-[#1A1A1A] text-white font-bold rounded-lg py-3 flex justify-center items-center gap-2 transition-all duration-300 shadow-sm mt-2 hover:bg-primary disabled:opacity-70 disabled:cursor-not-allowed"
              type="submit"
            >
              {loading ? (
                 <div className="flex items-center gap-2">
